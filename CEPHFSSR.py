...
def monkeyPatchedNormalizeType(type):
    ...
    if type in ["lvm", "lvmoiscsi", "lvmohba", "lvmofcoe"]:
        type = cleanup.SR.TYPE_LVHD
    # added "cephfs" to this list
    if type in ["ext", "nfs", "ocfsoiscsi", "ocfsohba", "smb", "cephfs"]:
...
cleanup.normalizeType = monkeyPatchedNormalizeType
...
CONFIGURATION = [['server', 'Ceph server(s) (required, ex: "192.168.0.12" or "10.10.10.10,10.10.10.26")'],
...
class CephFSSR(FileSR.FileSR):
    ...
    def handles(sr_type):
        ...
        return sr_type == 'cephfs' or sr_type == 'smb'
...
class CephFSFileVDI(FileSR.FileVDI):
    def attach(self, sr_uuid, vdi_uuid):
        if not hasattr(self, 'xenstore_data'):
            self.xenstore_data = {}
        self.xenstore_data["storage-type"] = "cephfs"
